...
CONFIG = {
    "testname": "Printer Text Output",            # projects main parent dir name, also used for testlist sorting
    "projdir": "printertest", 
    "cmainfile": "printmain",           # c-file progname no extenion to give to compiler
    "testtype": "CC65_C",                # name for this test type, used to make new run-button of like-named tests
...
@register_mytest(testtype, "Check out file")
def build3_screenshot_both(context):
    searchphrase = "TEST1"
    printfilepath = "/tmp/viceprnt.txt"
    log = []
    
    with open(printfilepath, "r") as f:
        if searchphrase in f.read():
            log.append(f"Phrase found in printerfile: {searchphrase} \n {printfilepath}")
            for vice_name in ["vice1"]:
                for name, instance in context.items():
                    if isinstance(instance, ViceInstance):
                        instance.take_screenshot()
        else:
            log.append("printer text search not success")
            context["abort"] = True
