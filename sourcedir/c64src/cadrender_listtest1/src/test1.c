...
int main(void){
  ...
  //  while (!kbhit()) { }
    ...
    for (i = 0; i < n; i++) {
        p_idx = load_object("PRYM,P,R", device);
        if (p_idx >= 0) {
            objects[p_idx]->ox = -80 + (i * 30);
            objects[p_idx]->oy = (i * 22);
            objects[p_idx]->oz = (i * 32);
            objects[p_idx]->rx = 0;
            objects[p_idx]->ry = 0;
            objects[p_idx]->rz = 0;
            printf("loaded: %d pts %d tris\n",
                objects[p_idx]->model->num_points,
                objects[p_idx]->model->num_tris
            );
            add_to_scene(objects[p_idx], 130, 40, 50, scnRotX, scnRotY, scnRotZ);
    ...
    //shift all the loaded objects by 15px on x
        for (obj_i = 0; obj_i < object_count; ++obj_i) {
        if (objects[obj_i])
            objects[obj_i]->ox += 15;
    }
