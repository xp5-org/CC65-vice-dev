...
//     if (!m) return -1;
//     // need to get actual face count
//     // Allocate memory for this specific model
//     pts = malloc(sizeof(Point3D) * MAX_POINTS);
//     trs = malloc(sizeof(FaceTri) * MAX_FACES);
//     if (!pts || !trs) {
//         free(m);
//         return -1;
//     }

//     // Load the model into these buffers
//     if (!load_model(name, device, m, pts, trs)) {
//         free(m);
//         free(pts);
//         free(trs);
//         return -1;
...
//     return object_count - 1; // return index
// }
...
int load_object(const char *name, unsigned char device)
{
    ...
    newpts = realloc(pts, sizeof(Point3D) * m->num_points);
    newtrs = realloc(trs, sizeof(FaceTri) * m->num_tris);
    if (!newpts || !newtrs) {
        free(newpts ? newpts : pts);
        free(newtrs ? newtrs : trs);
        free(m);
        return -1;
    }
